...
public class GetCourseInfoTests : MockConfiguration
{
    [Test]
    public async Task WhenSuccess()
    {
        ...
        var expected = _mapper.Map<CourseFormModel>(course);

        // Act
        var result = await _courseService.GetCourseInfoAsync(courseId);

        // Assert
        Assert.Multiple(() =>
        {
            Assert.That(result, Is.EqualTo(expected));
            Assert.IsInstanceOf<IList<CourseModuleFormModel>>(result.Modules);
    ...
    public void WhenFail_WrongCourseId_ThrowTest()
    {
        ...
        // Act & Assert
        Assert.ThrowsAsync<NullReferenceException>(async () => await _courseService.GetCourseInfoAsync(courseId));
    }
