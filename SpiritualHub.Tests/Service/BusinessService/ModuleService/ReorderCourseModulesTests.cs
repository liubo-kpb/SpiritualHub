...
public class ReorderCourseModulesTests : MockConfiguration
{
    ...
    [Test]
    public void WhenStartingFromMiddle()
    {
        // Arrange
        var modules = new[] { new Module() { Name = "a", Number = 1 }, new Module() { Name = "b", Number = 2 }, new Module() { Name = "c", Number = 3 }, new Module() { Name = "f", Number = 7 }, new Module() { Name = "d", Number = 4 }, new Module() { Name = "e", Number = 5 }, new Module() { Name = "g", Number = 4 } };
        int startingNumber = 4;

        // Act
        _moduleService.ReorderCourseModules(modules.Where(m => m.Number >= startingNumber), startingNumber);
        var result = modules.OrderBy(m => m.Number).ToArray();
